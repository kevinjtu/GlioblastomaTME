...
bulkSimulator_homo<-function(scExpr,scMeta,
  ...
  # --- BEGIN NEW DEBUGGING PRINTS for bulkSimulator_homo ---
  cat("\n--- DEBUG: ENTERING bulkSimulator_homo ---\n")
  cat("Class of scMeta received:\n")
  print(class(scMeta))
  cat("Dimensions of scMeta received (rows, cols):\n")
  print(dim(scMeta))
  cat("Head of scMeta received:\n")
  print(head(scMeta)) # Print head to see its structure and column names
  cat("Value of colnames_of_cellType argument:", colnames_of_cellType, "\n")
  
  if (colnames_of_cellType %in% names(scMeta)) {
    cat(paste0("Column '", colnames_of_cellType, "' IS present in scMeta.\n"))
    cell_type_labels_vector <- scMeta[[colnames_of_cellType]]
    cat("First few values of cell_type_labels_vector:\n")
    print(head(cell_type_labels_vector))
    cat("Table of cell_type_labels_vector:\n")
    print(table(cell_type_labels_vector, useNA = "ifany")) # Show NAs if any
    cat("Unique values from cell_type_labels_vector:\n")
    print(unique(cell_type_labels_vector))
  } else {
    cat(paste0("ERROR CRITICAL: Column '", colnames_of_cellType, "' IS NOT present in scMeta received by bulkSimulator_homo.\n"))
    cat("Available columns in scMeta are:\n")
    print(colnames(scMeta))
    # This would be a major issue, so stop if the column isn't there.
    stop(paste0("Required column '", colnames_of_cellType, "' not found in scMeta inside bulkSimulator_homo."))
  }
  ...
  # --- BEGIN DEBUGGING PRINTS ---
  cat("\n--- DEBUG: INSIDE bulkSimulator_homo ---\n")
  cat("colnames(simulated_frac) received by bulkSimulator_homo:\n")
  print(colnames(simulated_frac))
  cat("unique(cell_type_labels) from scMeta_test in bulkSimulator_homo:\n")
  print(unique(cell_type_labels))
  
  sim_cols <- sort(colnames(simulated_frac))
  meta_cols <- sort(unique(cell_type_labels))
  cat("Sorted colnames(simulated_frac):\n")
  print(sim_cols)
  cat("Sorted unique(cell_type_labels):\n")
  print(meta_cols)
  cat("Are they identical? ", identical(sim_cols, meta_cols), "\n")
  cat("Elements in sim_cols NOT in meta_cols:\n")
  print(setdiff(sim_cols, meta_cols))
  cat("Elements in meta_cols NOT in sim_cols:\n")
  print(setdiff(meta_cols, sim_cols))
...
bulkSimulator = function(methods,
  ...
  # --- NEW DEBUGGING PRINTS for bulkSimulator (wrapper) ---
  cat("\n--- DEBUG: ENTERING bulkSimulator (wrapper function) ---\n")
  cat("Class of scMeta received by bulkSimulator wrapper:\n")
  print(class(scMeta))
  cat("Dimensions of scMeta received by bulkSimulator wrapper (rows, cols):\n")
  print(dim(scMeta))
  cat("Head of scMeta received by bulkSimulator wrapper:\n")
  print(head(scMeta))
  if (is.data.frame(scMeta) && "colnames_of_cellType" %in% names(scMeta)) { # Check if it has the column by the literal name
    cat("Table of 'colnames_of_cellType' in scMeta within bulkSimulator wrapper:\n")
    print(table(scMeta[["colnames_of_cellType"]]))
  } else if (is.data.frame(scMeta) && colnames_of_cellType %in% names(scMeta)) { # Check by variable
    cat("Table of scMeta[[colnames_of_cellType]] in scMeta within bulkSimulator wrapper:\n")
    print(table(scMeta[[colnames_of_cellType]]))
  } else {
    cat("scMeta in bulkSimulator wrapper does not seem to have the expected cell type column or is not a data frame.\n")
  }
  cat("--- END DEBUG bulkSimulator wrapper ---\n\n")
  # --- END NEW DEBUGGING PRINTS ---
