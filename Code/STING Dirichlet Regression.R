...
res <- DirichReg(Y ~ STING_s + study_f, data = df_subset, model="alternative", base = 1)
summary(res)
...
combined_df <- imap_dfr(res_list, function(fit, subtype) {
  ...
  idx   <- grep("STING_s", names(coefs), fixed = TRUE)
  ...
  cell  <- sub("^STING_s[\\._]", "", names(coefs)[idx])
...
combined_df$cell <- sub(":STING_s$", "", combined_df$cell)
