...
# To make Figure S7 BANKSY UMAP plots 
...
# Make Volcano Plots Of BANKSY DEGs found in perinecrotic niche #
# Label genes that are part of perinecrotic niche gene signature in plot #
ls() ## list objects currently present in workspace
rm(list=ls()) ## remove objects from current workspace
ls() ## confirm current workspace is empty
library(EnhancedVolcano) # version 1.16.0
# Read in data frame with DEGs in perinecrotic niche of each sample
# Store in variable called res
# DEGs will have padjust < 0.05 and Log2FC < -0.5 or > 0.5
res <-read.table(file.choose(), sep = ",", header = TRUE, row.names = 1)
head(res) # Look at first 6 rows of data in res
# Make volcano plot
EnhancedVolcano(res,
                lab = rownames(res),
                x = 'log2FoldChange',
                ...
                selectLab = c('CAV1', 'CXCL8', 'HILPDA', 'VEGFA', 'LOX',
                              'VIM', 'TNC', 'OLIG1', 'FLT1', 
                              'KDR', 'PECAM1', 'TMEM173',
                              'BCAN'),
                pCutoff = 0.05,
